...
async function loadMenu() {
  try {
    ...
    document.getElementById('menuMessage').innerText = 'Failed to load menu.';
    console.error(err);
  }
}
...
function closeAuthModal() { document.getElementById('authModal').setAttribute('aria-hidden','true'); }
...
document.getElementById('formLogin').onsubmit = async e => {
  ...
  if(data.success){
    ...
    document.getElementById('usernameShow').innerText = currentUser.Cus_Name;
...
document.getElementById('checkoutBtn').onclick = async() => {
  ...
  if(!currentUser){ alert('Login first to checkout'); return; }
  ...
  const data = await res.json();
  if(data.success){
    alert(`Thank you ${currentUser.Cus_Name}! Your order of $${total.toFixed(2)} has been placed.`);
    cart=[]; renderCart();
  }
